...
public class AddBlackBeltController implements Initializable {
	...
	@FXML
	private TextField degreeNum;
	@FXML
	private TextField starNum;
	@FXML
	...
	private void addColorBeltStudent(ActionEvent event1)
	{
		...
		try
		{
			...
			if(!testResult.next())
			{
			    try
			    {
				String sqlInsert = "INSERT INTO Students(fname, lname, guardian, age, testingDate, address, phoneNumber, email, attendance, degree, numOfStars, beltColor, beltStripeColor) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)";
				...
				statement.setInt(10, Integer.parseInt(this.degreeNum.getText()));
				statement.setInt(11, Integer.parseInt(this.starNum.getText()));
				statement.setString(12, "Black");
				...
				while(rs.next())
				{
					Student joe = new Student(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getInt(5), rs.getString(6), rs.getInt(7), rs.getString(8), rs.getString(9), rs.getString(10), rs.getString(11), rs.getInt(12), rs.getInt(13), rs.getString(14));
					idSaver = joe.getId();
				}*/
				//conn1.close();
				
				
				/*String sqlInsertIntoColorBelt = "INSERT INTO BlackBelt(id, degree, numOfStars) VALUES(?,?,?) " ;
				...
				statement2.setInt(2, Integer.parseInt(this.degreeNum.getText()));
				statement2.setInt(3, Integer.parseInt(this.starNum.getText()));
				statement2.execute();
				conn.close();
				//conn1.close();
				//conn2.close();*/
	...
	public void clearForm(ActionEvent event4)
	{
		...
		this.email.setText("");
		this.degreeNum.setText("");
		this.starNum.setText("");
	}
